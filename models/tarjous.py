...
class Tarjous:
    ...
    def add_sample_offers():
        """Add sample offers to the database"""
        sample_offers = [
            {
                'title': 'iPhone 15 Pro - Exclusive Offer',
                'description': 'Latest iPhone 15 Pro with amazing camera and performance. Limited time offer with substantial savings.',
                'imageUrl': 'https://example.com/iphone15pro.jpg',
                'categoryName': 'Electronics',
                'merchantName': 'TechStore Helsinki',
                'merchantAddress': 'Aleksanterinkatu 15, Helsinki',
                'merchantPhone': '+358 9 1234567',
                'merchantWebsite': 'https://techstore.fi',
                'validFrom': datetime.now(),
                'validUntil': '2024-12-31',
                'terms': 'Valid for new customers only. Limited stock available.'
            },
            {
                'title': 'Samsung Galaxy S24 Ultra',
                'description': 'Experience the latest Samsung Galaxy S24 Ultra with AI features and excellent camera quality.',
                'imageUrl': 'https://example.com/galaxys24.jpg',
                'categoryName': 'Electronics',
                'merchantName': 'Mobile World Tampere',
                'merchantAddress': 'Hämeenkatu 21, Tampere',
                'merchantPhone': '+358 3 9876543',
                'merchantWebsite': 'https://mobileworld.fi',
                'validFrom': datetime.now(),
                'validUntil': '2024-11-30',
                'terms': 'Trade-in program available for additional discounts.'
            },
            {
                'title': 'MacBook Air M3 - Student Discount',
                'description': 'Perfect laptop for students and professionals. M3 chip provides incredible performance and battery life.',
                'imageUrl': 'https://example.com/macbookair.jpg',
                'categoryName': 'Computers',
                'merchantName': 'Apple Store Turku',
                'merchantAddress': 'Yliopistonkatu 5, Turku',
                'merchantPhone': '+358 2 5555555',
                'merchantWebsite': 'https://apple.com/fi',
                'validFrom': datetime.now(),
                'validUntil': '2024-12-15',
                'terms': 'Student ID required. Educational discount applies.'
            },
            {
                ...
                'categoryName': 'Audio',
                'merchantName': 'Sound Paradise Oulu',
                'merchantAddress': 'Rotuaari 12, Oulu',
                'merchantPhone': '+358 8 7777777',
                'merchantWebsite': 'https://soundparadise.fi',
                'validFrom': datetime.now(),
                'validUntil': '2024-11-25',
                'terms': 'Free shipping included. 2-year warranty.'
            },
            {
                'title': 'Nike Air Max 270 - Fall Collection',
                'description': 'Comfortable and stylish sneakers perfect for everyday wear. Available in multiple colors.',
                'imageUrl': 'https://example.com/nikeairmax.jpg',
                'categoryName': 'Fashion',
                'merchantName': 'SportZone Espoo',
                'merchantAddress': 'Kauppakeskus Iso Omena, Espoo',
                'merchantPhone': '+358 9 3333333',
                'merchantWebsite': 'https://sportzone.fi',
                'validFrom': datetime.now(),
                'validUntil': '2024-12-10',
                'terms': 'All sizes available. Exchange policy within 30 days.'
            }
        ]
        ...
        try:
            for offer_data in sample_offers:
                ...
                with get_db_connection() as conn:
                    with conn.cursor() as cursor:
                        cursor.execute("SELECT COUNT(*) FROM tarjoukset WHERE title = %s", (offer_data['title'],))
                        if cursor.fetchone()[0] == 0:
                            Tarjous.create_offer(offer_data)
            
            print('✅ Sample offers added successfully')
        except Exception as error:
            print(f'❌ Error adding sample offers: {error}')
            raise error 